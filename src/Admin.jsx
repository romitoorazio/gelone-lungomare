...
export default function Admin() {
    ...
    missingContactRows.forEach((booking) => {
      actionRows.push({
        ...
        detail:
          ...
          (booking.missingPhone && booking.missingEmail ? " Â· " : "") +
  ...
  function exportActivityLogsCsv(mode = "all") {
    ...
    if (ok) {
      setMessage("Export log attivitÃ  creato: " + rows.length + " righe.");
  ...
  async function createInternalRecord(event) {
    ...
    if (note.length < 5) {
      setError("Inserisci una nota interna piÃ¹ dettagliata.");
      return;
    }
  ...
  async function deleteSelectedUnit() {
    ...
    if (!id || !unitToDelete) {
      setError("Seleziona una unitÃ  valida da eliminare.");
      return;
    }

    if (id === UNIT_ID) {
      setError("Non puoi eliminare l'unitÃ  principale Lunarossa 1. Puoi solo modificarla o disattivarla.");
      return;
    }
    ...
    const confirmed = window.confirm(
      "Vuoi eliminare definitivamente questa unitÃ ?\n\n" +
        "UnitÃ : " +
        unitName +
        "\nID: " +
        id +
        "\n\nL'unitÃ  sparirÃ  dall'admin e dal calendario multi-alloggio."
    );

    if (!confirmed) {
      setMessage("Eliminazione unitÃ  annullata.");
      return;
    }
    ...
    if (nextUnits.length === 0) {
      setError("Non puoi eliminare tutte le unitÃ . Deve restare almeno un alloggio.");
      return;
    }
    ...
    try {
      ...
      setMessage("UnitÃ  eliminata: " + unitName + ".");
    } catch (err) {
      console.error(err);
      setError("Errore durante l'eliminazione dell'unitÃ .");
    }
  }
...
wifiName: settings.wifiName || "",
  ...
  async function cleanupGhostNights() {
    setError("");
    setCleanupResult(null);
    setMessage("Pulizia notti fantasma disattivata sul piano Vercel Hobby. Prenotazioni, calendario, email e pagamenti restano attivi.");
    return;
    clearMessages();
  ...
  async function cancelBooking(booking) {
    ...
    const reason = window.prompt(
      "Motivo obbligatorio per annullare la prenotazione di " +
        ...
        ".\n\nQuesta azione libera le notti e modifica la disponibilitÃ  pubblica." +
      ...
      if (datesChanged) {
        ...
        const externalWarning =
          ["booking", "airbnb", "booking_ical", "airbnb_ical", "imported_ical"].includes(
            ...
            ? "\n\nATTENZIONE: se arriva da Booking/Airbnb, modifica anche lÃ¬. Una sincronizzazione futura potrebbe rimettere le vecchie date."
            : "";
  ...
  async function setManualPaymentStatus(booking, paymentStatus, manualAction) {
    ...
    if (currentPaymentStatus === "paid" && nextPaymentStatus === "deposit_paid") {
      setError("Controllo anti-errore: questa prenotazione Ã¨ giÃ  saldata, non puoi segnarla solo come caparra.");
      return;
    }
    ...
    const confirmed = window.confirm(
      "Confermi aggiornamento pagamento manuale?\n\n" +
        ...
        "\n\nIl movimento sarÃ  salvato nel log attivitÃ ."
    );
...
<TabButton active={activeTab === "calendar"} onClick={() => setActiveTab("calendar")}>
                    ...
                    {structureStats.cleaningRows.slice(0, 10).map((booking) => (
                      <div key={booking.id} className="rounded-2xl border border-[#e4d8c2] bg-white p-4">
                        <div className="font-bold text-[#0a1d35]">
                          {formatDate(booking.checkOut)} Â· {booking.unitName}
                        </div>
                        <div className="mt-1 text-sm text-[#555]">
                          {booking.guestName || "Ospite"} Â· {getPreparationLabel(booking.preparationStatus)}
                        </div>
                        <div className="mt-3">
                          <SmallButton
                            onClick={() => {
        ...
        {activeTab === "availability" && (
          <section className="mt-8 space-y-6">
            <div className="overflow-hidden rounded-[2.2rem] border border-[#d8c7a6] bg-white shadow-xl shadow-[#0a1d35]/5">
              <div className="bg-gradient-to-r from-[#0a1d35] via-[#12345a] to-[#9b6b25] p-6 text-white">
                <div className="flex flex-col gap-5 lg:flex-row lg:items-end lg:justify-between">
                  <div>
                    <p className="text-xs uppercase tracking-[0.35em] text-[#f6d899]">
                    ...
                    <p className="mt-3 max-w-3xl leading-7 text-white/80">
                      Vista professionale di disponibilitÃ , occupazioni, richieste e blocchi.
                  ...
                  <div className="rounded-[1.4rem] border border-white/20 bg-white/10 p-4 backdrop-blur">
                    <div className="flex flex-wrap items-center gap-3">
                      <button
                        type="button"
                        onClick={() => {
                          const base = parseDateAsUTC((availabilityCalendar.month || getToday().slice(0, 7)) + "-01");
                          base.setUTCMonth(base.getUTCMonth() - 1);
                          const nextMonth = base.toISOString().slice(0, 7);
                      ...
                      <input
                        ...
                        className="rounded-full border border-white/30 bg-white px-5 py-3 font-bold text-[#0a1d35]"
                      />
                      ...
                      <button
                        ...
                        className="rounded-full bg-[#f6d899] px-5 py-3 font-bold text-[#0a1d35] transition hover:bg-white"
                      >
                        Oggi
                      </button>
                    </div>
              ...
              <div className="border-b border-[#eadcc2] bg-[#faf6ee] p-5">
                ...
                <div className="mt-5 flex flex-wrap items-center gap-3 text-xs font-bold">
                  <span className="rounded-full border border-green-200 bg-green-50 px-4 py-2 text-green-900">
                    Libero
                  </span>
                  <span className="rounded-full border border-red-200 bg-red-50 px-4 py-2 text-red-900">
                    Occupato / confermato
                  </span>
                  <span className="rounded-full border border-amber-300 bg-amber-50 px-4 py-2 text-amber-900">
                    Richiesta da confermare
                  </span>
                  <span className="rounded-full border border-slate-300 bg-slate-100 px-4 py-2 text-slate-900">
                    Blocco manuale
                  </span>
                  <span className="rounded-full border border-[#d7c49f] bg-white px-4 py-2 text-[#0a1d35]">
                    Riga evidenziata = giorno selezionato
                  </span>
                </div>
              </div>

              <div className="p-5">
                <div className="overflow-hidden rounded-[1.8rem] border border-[#d8c7a6] bg-white">
                  <div className="max-h-[640px] overflow-auto">
                    <table className="w-full min-w-[1180px] border-separate border-spacing-0 text-left text-sm">
                      <thead>
                        <tr className="sticky top-0 z-20 bg-[#0a1d35] text-white shadow">
                          <th className="sticky left-0 z-30 w-36 bg-[#0a1d35] px-4 py-4">
                            Data
                          </th>

                          {availabilityCalendar.units.map((unit) => (
                            <th key={unit.id} className="min-w-[210px] px-4 py-4">
                              <div className="flex items-center gap-2">
                                <Building2 size={16} className="text-[#f6d899]" />
                                <span className="font-bold">{unit.displayName}</span>
                              </div>
                            </th>
                          ))}
                        </tr>
                      ...
                      <tbody>
                        {availabilityCalendar.rows.map((row) => {
                          const dayLabel = formatDate(row.date);
                          const allFree = row.freeCount === availabilityCalendar.units.length;
                          ...
                          return (
                            <tr
                              ...
                              className={
                                ...
                                (row.isSelected ? "bg-[#fff2ce]" : allFree ? "bg-green-50/30" : "bg-white")
                              }
                            >
                              <td className="sticky left-0 z-10 border-r border-[#eadcc2] bg-inherit px-4 py-3 align-top">
                                <button
                                  type="button"
                                  onClick={() => {
                                  ...
                                  className={
                                    ...
                                    (row.isSelected
                                      ? "bg-[#0a1d35] text-white shadow"
                                      : "bg-white text-[#0a1d35] hover:bg-[#faf6ee]")
                                  }
                                >
                                  <div className="text-base font-black">{row.label}</div>
                                  <div className="mt-1 text-xs font-semibold opacity-75">
                                    {dayLabel}
                                  </div>
                                  {row.isToday && (
                                    <div className="mt-2 inline-flex rounded-full bg-[#f6d899] px-2 py-1 text-[10px] font-black text-[#0a1d35]">
                                      OGGI
                                    </div>
                                  )}
                                </button>
                              ...
                              {row.cells.map((cell) => {
                                ...
                                const isStart = booking && String(booking.checkIn || "") === row.date;
                                const isEndPreview = booking && String(booking.checkOut || "") === row.date;
                                const guest = booking?.guestName || booking?.notes || "Prenotazione";
                                const price = Number(booking?.totalPrice || 0);
                                const sourceLabel = booking ? getSourceLabel(booking.source) : "";

                                return (
                                  <td key={cell.unit.id} className="border-b border-[#f6eddc] px-3 py-3 align-top">
                                    <button
                                      type="button"
                                      onClick={() => {
                                        setAvailabilitySelectedDate(row.date);
                                      ...
                                      className={
                                        "min-h-[88px] w-full rounded-[1.2rem] border px-3 py-3 text-left shadow-sm transition hover:-translate-y-0.5 hover:shadow-md " +
                                        cell.className
                                      }
                                    >
                                      <div className="flex items-start justify-between gap-2">
                                        <div>
                                          <div className="text-sm font-black">{cell.label}</div>
                                          <div className="mt-1 text-[11px] font-bold opacity-70">
                                            {booking ? sourceLabel : "Disponibile"}
                                          </div>
                                        </div>
                ...
                <div className="mt-8 grid gap-5 xl:grid-cols-[0.9fr_1.4fr]">
                  <div className="rounded-[1.8rem] border border-[#d8c7a6] bg-[#0a1d35] p-6 text-white shadow-sm">
                    <p className="text-xs uppercase tracking-[0.3em] text-[#f6d899]">
                      Giorno selezionato
                    </p>
                    ...
                    <div className="mt-6 grid grid-cols-2 gap-3">
                      <div className="rounded-2xl bg-white/10 p-4">
                        <div className="text-3xl font-black">
                          {availabilityCalendar.selectedRows.filter((row) => !row.booking).length}
                        </div>
                        <div className="text-sm text-white/70">Alloggi liberi</div>
                      </div>

                      <div className="rounded-2xl bg-white/10 p-4">
                        <div className="text-3xl font-black">
                          {availabilityCalendar.selectedRows.filter((row) => row.booking && String(row.booking.status || "") !== "blocked").length}
                        </div>
                        <div className="text-sm text-white/70">Occupati</div>
                      </div>

                      <div className="rounded-2xl bg-white/10 p-4">
                        <div className="text-3xl font-black">
                          {availabilityCalendar.selectedRows.filter((row) => row.booking && ["pending_direct", "pending"].includes(String(row.booking.status || ""))).length}
                        </div>
                        <div className="text-sm text-white/70">Richieste</div>
                      </div>

                      <div className="rounded-2xl bg-white/10 p-4">
                        <div className="text-3xl font-black">
                          {availabilityCalendar.selectedRows.filter((row) => String(row.booking?.status || "") === "blocked").length}
                        </div>
                        <div className="text-sm text-white/70">Blocchi</div>
                      </div>
                    </div>
                  </div>

                  <div className="rounded-[1.8rem] border border-[#d8c7a6] bg-[#faf6ee] p-5">
                    <div className="flex flex-col gap-2 md:flex-row md:items-center md:justify-between">
                      <div>
                        ...
                        <h3 className="mt-2 text-2xl font-bold text-[#0a1d35]">
                          Dettaglio disponibilitÃ  e prenotazioni
                        </h3>
                      </div>
                    </div>
                    ...
                    <div className="mt-5 grid gap-4 lg:grid-cols-2">
                      {availabilityCalendar.selectedRows.map((row) => {
                        ...
                        return (
                          <div
                            key={row.unit.id}
                            className="overflow-hidden rounded-[1.4rem] border border-[#e4d8c2] bg-white shadow-sm"
                          >
                            <div className={"border-b px-5 py-4 " + row.className}>
                            ...
                            {!booking ? (
                              <div className="p-5">
                                <div className="rounded-2xl bg-green-50 p-4 text-sm font-semibold leading-6 text-green-900">
                                  Libero per questa data. Nessuna prenotazione o blocco registrato.
                                </div>
                              </div>
                            ) : (
                              <div className="space-y-4 p-5 text-sm text-[#0a1d35]">
                                ...
                                <div className="flex flex-wrap gap-2">
                                  ...
                                  {whatsappNumber && booking.status !== "blocked" && (
                                    <a
                                      href={"https://wa.me/" + whatsappNumber + "?text=" + buildWhatsAppMessage(booking, settings)}
                                      target="_blank"
                                      rel="noreferrer"
          ...
          <section className="mt-8 space-y-6">
            <div className="rounded-[2rem] border border-[#e4d8c2] bg-white p-6 shadow-sm">
              ...
              <p className="mt-3 max-w-3xl leading-7 text-[#555]">
                ...
                e promemoria interni. Ogni nota resta nello storico e puÃ² essere chiusa o riaperta.
              </p>
                      ...
                      {bookings
                        ...
                        .map((booking) => (
                          <option key={booking.id} value={booking.id}>
                            {booking.guestName || "Prenotazione"} Â· {formatDate(booking.checkIn)} - {formatDate(booking.checkOut)}
                          </option>
                        ))}
                    </select>
                  </FormField>
                  ...
                  <div className="md:col-span-3">
                    <FormField label="Nota dettagliata">
                      <textarea
                        ...
                        placeholder="Scrivi cosa Ã¨ successo, cosa va fatto, chi deve controllare o cosa Ã¨ stato deciso."
              ...
              <div className="mt-8 rounded-[1.5rem] border border-[#e4d8c2] bg-[#faf6ee] p-5">
                        ...
                        <tr key={record.id} className="border-b border-[#f0e6d5] align-top">
                          ...
                          <td className="py-4">
                            {record.linkedGuestName
                              ? record.linkedGuestName + " Â· " + formatDate(record.linkedCheckIn)
                              : "-"}
                          </td>
                ...
                <div className="mt-5 rounded-2xl border border-amber-200 bg-amber-50 p-4 text-sm leading-6 text-amber-900">
                  Il registro interno non modifica disponibilitÃ , prenotazioni o pagamenti.
          ...
          <section className="mt-8 space-y-6">
            <div className="rounded-[2rem] border border-[#e4d8c2] bg-white p-6 shadow-sm">
              ...
              <p className="mt-3 max-w-3xl leading-7 text-[#555]">
                ...
                Il prossimo arrivo viene mostrato solo se Ã¨ vicino, entro 7 giorni, cosÃ¬ non mischia prenotazioni lontane.
              </p>
              ...
              <div className="mt-8 rounded-[1.5rem] border border-[#e4d8c2] bg-[#faf6ee] p-5">
                ...
                <div className="mt-5 rounded-2xl border border-amber-200 bg-amber-50 p-4 text-sm leading-6 text-amber-900">
                  ...
                  Non modifica calendario, pagamenti o disponibilitÃ  pubblica.
                </div>
              </div>
            </div>
          </section>
          ...
          <section className="mt-8 space-y-6">
            <div className="rounded-[2rem] border border-[#e4d8c2] bg-white p-6 shadow-sm">
              ...
              <p className="mt-3 max-w-3xl leading-7 text-[#555]">
                ...
                WelcoMate, qualitÃ  dati, telefono e note operative.
              </p>
        ...
        {activeTab === "quality" && (
          <section className="mt-8 space-y-6">
            <div className="rounded-[2rem] border border-[#e4d8c2] bg-white p-6 shadow-sm">
              ...
              <p className="mt-3 max-w-3xl leading-7 text-[#555]">
                Questa sezione controlla la qualitÃ  operativa delle prenotazioni:
        ...
        {activeTab === "backup" && (
          <section className="mt-8 space-y-6">
            <div className="rounded-[2rem] border border-[#e4d8c2] bg-white p-6 shadow-sm">
              ...
              <p className="mt-3 max-w-3xl leading-7 text-[#555]">
                Scarica copie CSV delle prenotazioni e del registro attivitÃ .
              ...
              <div className="mt-6 grid gap-4 md:grid-cols-5">
                ...
                <StatCard title="Log attivitÃ " value={backupStats.activityLogs} icon={Search} subtitle="Ultimi log caricati" />
              ...
              <div className="mt-8 grid gap-5 lg:grid-cols-2">
                <div className="rounded-[1.5rem] border border-[#e4d8c2] bg-[#faf6ee] p-5">
                  ...
                  <p className="mt-2 text-sm leading-6 text-[#555]">
                    Esporta i record prenotazione dell'unitÃ  selezionata con date,
              ...
              <div className="mt-8 rounded-[1.5rem] border border-[#e4d8c2] bg-[#faf6ee] p-5">
                <div className="flex flex-col gap-3 md:flex-row md:items-end md:justify-between">
                  <div>
                    ...
                    <p className="mt-2 text-sm leading-6 text-[#555]">
                      Le urgenze sono vicine al check-in o delicate. Le attivitÃ  future restano visibili ma separate, cosÃ¬ non sembrano emergenze di oggi.
                    </p>
                  </div>
                </div>
                      ...
                      {controlsStats.urgentActionRows.length > 0 && (
                        <tr>
                          <td colSpan="6" className="bg-red-50 px-4 py-3 font-bold text-red-900">
                            Urgenti Â· da controllare subito
                          </td>
                        </tr>
                      )}
                      ...
                      {controlsStats.futureActionRows.length > 0 && (
                        <tr>
                          <td colSpan="6" className="bg-amber-50 px-4 py-3 font-bold text-amber-900">
                            Futuri Â· da tenere sotto controllo
                          </td>
                        </tr>
                      )}
          ...
          <section className="mt-8 space-y-6">
            <div className="rounded-[2rem] border border-[#e4d8c2] bg-white p-6 shadow-sm">
              ...
              <p className="mt-3 max-w-3xl leading-7 text-[#555]">
                ...
                e valore medio delle prenotazioni dell'unitÃ  selezionata.
              </p>
          ...
          <section className="mt-8 space-y-6">
            <div className="rounded-[2rem] border border-[#e4d8c2] bg-white p-6 shadow-sm">
              ...
              <div className="mt-5 rounded-2xl border border-amber-200 bg-amber-50 p-4 text-amber-900">
                Questo registro Ã¨ interno: serve per controllare le azioni operative
          ...
          <section className="mt-8 space-y-6">
            <div className="rounded-[2rem] border border-[#e4d8c2] bg-white p-6 shadow-sm">
              ...
              <p className="mt-3 max-w-3xl leading-7 text-[#555]">
                ...
                urgenze, attivitÃ  future, saldi, WelcoMate e stato sincronizzazione calendari.
              </p>
                  ...
                  <div className="mt-5 space-y-3">
                    ...
                    {(controlsStats.futureActionRows || []).length > 0 && (
                      <div className="rounded-2xl border border-amber-200 bg-amber-50 p-4 text-amber-900">
                        Ci sono attivitÃ  future da tenere sotto controllo.
                      </div>
                    )}
                      ...
                      <div className="rounded-2xl border border-red-200 bg-red-50 p-4 text-red-900">
                        ...
                        <div className="mt-2 space-y-1 text-sm">
                          {preparationStats.notReadyRows.slice(0, 4).map((booking) => (
                            ...
                            >
                              {formatDate(booking.checkOut)} Â· {booking.guestName || "Ospite"} Â· {getPreparationLabel(booking.preparationStatus)}
                            </button>
                          ))}
                        </div>
                      </div>
