...
async function verifyAdminRequest(req) {
  ...
  if (!token) {
    const error = new Error("Accesso admin richiesto per inviare email.");
  ...
  if (!ADMIN_EMAILS.includes(email)) {
    const error = new Error("Account non autorizzato a inviare email.");
...
function getSiteOrigin(req) {
  const configuredUrl = cleanText(process.env.PUBLIC_SITE_URL);
...
function buildTextEmail({ booking, unitName, paymentUrl, welcomateUrl }) {
  ...
  return [
    ...
    "- Negli ultimi 6 giorni, no-show o partenza anticipata: importi non rimborsabili salvo diverso accordo scritto.",
    "",
    "Contatti:",
    "WhatsApp: 3476308456",
    "Telefono: 3479461999",
...
function buildHtmlEmail({ booking, unitName, origin, paymentUrl, welcomateUrl }) {
  ...
  const paymentBox = paymentUrl
    ? '<div style="margin:26px 0;padding:22px;border-radius:18px;background:#0a1d35;text-align:center;">' +
        '<p style="margin:0 0 14px;color:#ffffff;font-size:16px;">Puoi completare il pagamento in modo sicuro dal link seguente.</p>' +
      ...
      '</div>'
    : '<div style="margin:26px 0;padding:20px;border-radius:18px;background:#faf6ee;border:1px solid #eadcc5;color:#0a1d35;">La struttura ti invierà le indicazioni per il pagamento, se dovuto.</div>';

  const welcomateBox = welcomateUrl
    ? '<tr><td style="padding:10px 0;color:#6b5b46;">Check-in ospiti</td><td style="padding:10px 0;text-align:right;"><a href="' + escapeHtml(welcomateUrl) + '" style="color:#0a1d35;font-weight:800;">Apri WelcoMate</a></td></tr>'
    : "";

  return '<!doctype html>' +
    '<html><body style="margin:0;background:#f4efe6;font-family:Arial,Helvetica,sans-serif;color:#0a1d35;">' +
    '<div style="max-width:720px;margin:0 auto;padding:28px 14px;">' +
      '<div style="background:#ffffff;border-radius:28px;overflow:hidden;border:1px solid #e4d8c2;box-shadow:0 18px 60px rgba(10,29,53,0.10);">' +
        '<div style="background:#0a1d35;padding:30px 24px;text-align:center;">' +
          ...
          '<h1 style="margin:12px 0 0;color:#ffffff;font-family:Georgia,serif;font-size:32px;line-height:1.1;">La tua richiesta è stata confermata</h1>' +
        '</div>' +

        '<div style="padding:30px 26px;">' +
          '<p style="font-size:18px;line-height:1.7;margin:0 0 18px;">Ciao <strong>' + guestName + '</strong>,</p>' +
          '<p style="font-size:16px;line-height:1.75;margin:0 0 22px;color:#4f5b67;">Abbiamo verificato la disponibilità e la tua richiesta per <strong>' + safeUnitName + '</strong> è confermata dalla struttura.</p>' +
  ...
  try {
    ...
    if (!resendApiKey) {
      return res.status(500).json({
        ok: false,
        message: "RESEND_API_KEY non configurata su Vercel.",
      });
    }
    ...
    const subject = emailType === "availability_confirmed"
