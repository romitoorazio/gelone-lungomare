...
function getNights(booking) {
  const explicit = Number(booking?.nightsCount || booking?.nights || 0);
...
async function sendInternalBookingEmail(booking) {
  ...
  const rows = [
    ...
    .map(
      ([label, value]) =>
        `<tr><td style="padding:13px 16px;border-bottom:1px solid #f0e6d5;color:#6b5b46;">${escapeHtml(label)}</td><td style="padding:13px 16px;border-bottom:1px solid #f0e6d5;text-align:right;font-weight:800;color:#0a1d35;">${escapeHtml(value)}</td></tr>`
    )
    .join("");

  const callButton = phoneDigits
    ? `<a href="tel:+${phoneDigits.startsWith("39") ? phoneDigits : `39${phoneDigits}`}" style="display:inline-block;margin:8px 6px;padding:13px 20px;border-radius:999px;background:#15803d;color:#fff;text-decoration:none;font-weight:800;">Chiama ospite</a>`
    : "";
...
export default async function handler(req, res) {
  ...
  try {
    ...
    const emailNotification = await sendBookingConfirmationEmail({
      bookingId,
      adminEmail: adminUser.email,
    });

    let internalEmailNotification = null;
    if (body.notifyInternal === true) {
      const adminDb = getFirebaseAdminDb();
      const snapshot = await adminDb.collection("bookings").doc(bookingId).get();
      internalEmailNotification = snapshot.exists
        ? await sendInternalBookingEmail({ id: snapshot.id, ...snapshot.data() })
        : { sent: false, reason: "Prenotazione non trovata per la notifica interna." };
    }
    ...
    console.error("Errore send-confirmation-email:", error);

    return json(res, error?.statusCode || 500, {
      ok: false,
      message: error?.message || "Errore durante invio email conferma.",
    });
  }
}
